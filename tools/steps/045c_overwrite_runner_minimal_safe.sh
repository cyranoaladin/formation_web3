#!/usr/bin/env bash
# STEP 045c — overwrite_runner_minimal_safe (CODE CHANGE)
# But: remplacer runner/minimal.py par une version sûre (sans f-string) et re-valider

set -euo pipefail

TS=$(date +%Y%m%d_%H%M%S)
LOG="tools/logs/045c_runner_overwrite_${TS}.txt"
mkdir -p tools/logs

RUNNER_MOD="runner/minimal.py"
FIXTURE="tests/fixtures/minimal.zip"
SANDBOX="/tmp/rbk_sandbox_045c"

sha() { if [[ -f "$1" ]]; then sha256sum "$1" | awk '{print $1}'; else echo "(absent)"; fi }

{
  echo "[INFO] STEP 045c @ ${TS}"
  echo "== SHA BEFORE =="
  echo "runner/minimal.py: $(sha "$RUNNER_MOD")"

  echo
  echo "== OVERWRITE runner/minimal.py =="
  python3 - <<'PY'
...
print('RUNNER_OVERWRITTEN_SAFE=1')
PY
  ...
  rm -rf "$SANDBOX" || true
  python3 -m runner.minimal --lab-id hello-proof --submission-id sub_045c_demo --zip "$FIXTURE" --sandbox "$SANDBOX" | sed -n '1,200p'
